...
var Server = function (options) {
  ...
  this.touchIndex = serverConfig.touchIndex || 'index.html';
  this.headers = serverConfig.headers || {};
  this.shortPath = serverConfig.shortPath || '';
  this.cache = serverConfig.cache ||3600;
...
Server.prototype = {
  serve: function (callback) {
    var iPort = this.port || 3000;
    Util.checkPortStatus(iPort, {
      host: this.host
    }, function (err, status) {
      if (err || status === 'open') {
        Util.freePort(function (e, port) {
          if (e) {
            port = iPort;
          }
          this.createServer.call(this, port, callback);
        }.bind(this));
      } else {
        this.createServer.call(this, iPort, callback);
      }
    }.bind(this));
  },

  createServer: function (port, callback) {
    var serveUrl = Util.urlJoin('http://localhost:' + port + '/', this.shortPath);
    var serverIpUrl = Util.urlJoin('http://' + this.ip + ':' + port + '/', this.shortPath);
    if (this.touchIndex !== 'index.html') {
      serveUrl = Util.urlJoin(serveUrl, this.touchIndex);
      serverIpUrl = Util.urlJoin(serverIpUrl, this.touchIndex);
    }
    var app = connect();
    ...
    app.use(function (req, res) {
      ...
      if (req.url.indexOf('??') > -1) {
        return process.nextTick(function () {
          var comboFn = combo({
    				root: path.join(this.root, this.shortPath),
    				cache: this.cache
    			});
